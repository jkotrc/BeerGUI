...
ButtonComponent::ButtonComponent(Point const &dimensions, Action action)
    : _action(action) {
...
void ButtonComponent::draw(Graphics &g) {
  ...
  Region inner = {_region.top_left + Point{1, 1},
                  _region.bottom_right - Point{1, 1}};
...
void ButtonComponent::activate() { _action(); }
