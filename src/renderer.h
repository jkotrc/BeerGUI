...
public:
  ...
  virtual void drawLine(Point const &from, Point const &to,
                        Color const &color) = 0;
  /// must be called once per loop to update display
  virtual void update() = 0;
  /// clear screen
  virtual void clear() = 0;
  // TODO void drawBitmap
  virtual void drawText(Point const &anchor, const char *text,
                        Color const &color) = 0;
};
 ...
 * ## How to customize
 *
 * To get the GUI working for a different display that depends on a different
 * hardware library, you will need to wrap the dependency in a class. With most
 * Adafruit displays, this is already done through the Adafruit_GFX library.
 * Otherwise (1) create a class to interface with the hardware library analogous
 * to GLBackend and (2) write a specialization for Renderer.
 *
 * Once finished with (1), declare this in your custom backend's header file
 * `myrenderlib.h`
 *
 * ``template <> using MyRenderer = Renderer<MyRenderLib>``
 *
 * then define all the functions from the interface in the sources
 * `myrenderlib.cpp`, for example
 *
 * ``template <> void MyRenderer::setup() { ... }``
 *
 */
template <typename RenderBackend> class Renderer : public Graphics {
public:
  Renderer(RenderBackend *backend) { _backend = backend; }
