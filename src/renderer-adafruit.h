...
void GFXRenderer::fill(Region const& region, Color const& color) {
    const width = region.bottom_right.x - region.top_left.x;
    const height = region.bottom_right.y - region.top_left.y;
    _backend->fillRect(region.top_left.x, region.top_left.y, width, height);
}
